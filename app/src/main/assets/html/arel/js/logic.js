...
arel.sceneReady(function() {
    ...
    scenario.currentTrackingConfigPathOrIdentifier = "html/resources/TrackingConfig_ad5c52fcb26a6596c5dd7ff2721d0a8b.zip";
    ...
    scenario.onStartup = function () {
        arel.Debug.log("Welcome to the 'Eiffeltoren' Augmented Reality experience.");
        ...
        scenario.registerObject(button5);
        if (methodExists(button5, button5.onLoaded)) {
            button5.onLoaded();
        }
        ...
        scenario.registerObject(contourTracking);
        if (methodExists(contourTracking, contourTracking.onLoaded)) {
            contourTracking.onLoaded();
        }
    ...
    if (!arel.Scene.objectExists("contourTracking")) {
        arel.Debug.log("ERROR: retrieving the object contourTracking, it seems that it does not exist. Maybe it would help cleaning the cache or a problem occurred while reloading the channel.");
    }

    var contourTracking = arel.Scene.getObject("contourTracking");
    scenario.trackables.push(contourTracking);
    contourTracking.objectName = "contourTracking";
    contourTracking.cosName = "TrackingPose";
    contourTracking.cosID = "1";
    contourTracking.isCurrentlyTracking = false;
    contourTracking.currentTrackingValues = null;
    contourTracking.isLoaded = function () {
        return arel.Scene.objectExists("contourTracking");
    };

    contourTracking.bind = function (cosID) {
        arel.Debug.log(this.objectName + ".bind(" + cosID + ")");
        this.setCoordinateSystemID(cosID);
    };

    contourTracking.load = function () {
    ...
    contourTracking.unload = function () {
    ...
    contourTracking.display = function () {
        arel.Debug.log(this.objectName + ".display()");
    ...
    contourTracking.hide = function () {
        arel.Debug.log(this.objectName + ".hide()");
        this.setVisibility(false);
    };

    contourTracking.onTracked = function (trackingValues) {
    ...
    contourTracking.onTrackingLost = function (trackingValues) {
    ...
    contourTracking.onVisible = function () {
        arel.Debug.log(this.objectName + ".onVisible()");
        button5.hide();
    };

    contourTracking.onInvisible = function () {
        arel.Debug.log(this.objectName + ".onInvisible()");
        button5.display();
    };
    ...
    if (!arel.Scene.objectExists("button5")) {
        arel.Debug.log("ERROR: retrieving the object button5, it seems that it does not exist. Maybe it would help cleaning the cache or a problem occurred while reloading the channel.");
    }

    var button5 = arel.Scene.getObject("button5");
    button5.objectName = "button5";
    button5.type = "Button";
    button5.scene = scene1;
    button5.associatedTrackable = userDevice;
    button5.displayOnLoaded = false;
    scenario.contents.push(button5);

    button5.setScene = function (scene) {
        this.scene = scene;
        scenario.currentScene.display();
    };

    button5.isLoaded = function () {
        return arel.Scene.objectExists("button5");
    };

    button5.bind = function (cosID) {
        arel.Debug.log(this.objectName + ".bind(" + cosID + ")");
        this.setCoordinateSystemID(cosID);
    };

    button5.load = function () {
        arel.Debug.log(this.objectName + ".load()");
        if (!this.isLoaded()) {
            scenario.addObject(this);
        }
    ...
    button5.hide = function () {
        arel.Debug.log(this.objectName + ".hide()");
        this.setVisibility(false);
    };

    button5.attach = function (origin, offset) {
        ...
        if (origin.groupID) {
            if (this.groupID) {
                arel.GestureHandler.removeObject(button5);
            }
            arel.GestureHandler.addObject("button5", origin.groupID);
            this.setPickingEnabled(false);
        }
    };

    button5.play = function (animationName, animationLooped) {
    ...
    button5.onTouchStarted = function () {
        arel.Debug.log(this.objectName + ".onTouchStarted()");
        scenario.reloadTrackingConfiguration();
    };

    button5.onLoaded = function () {
    ...
    // Eiffel Tower Construction Time Lapse
    if (!arel.Scene.objectExists("video2")) {
        arel.Debug.log("ERROR: retrieving the object video2, it seems that it does not exist. Maybe it would help cleaning the cache or a problem occurred while reloading the channel.");
    }

    var video2 = arel.Scene.getObject("video2");
    video2.objectName = "video2";
    video2.type = "InPageVideo";
    video2.scene = scene1;
    video2.associatedTrackable = contourTracking;
    video2.displayOnLoaded = false;
    video2.playOnLoaded = false;
    scenario.contents.push(video2);

    video2.setScene = function (scene) {
        this.scene = scene;
        scenario.currentScene.display();
    };
    ...
    video2.play = function () {
        ...
        if (scenario.googleAnalytics) {
            scenario.googleAnalytics.logVideo("Eiffel Tower Construction Time Lapse");
        }
        if (methodExists(this, this.onPlayed)) {
            this.onPlayed();
        }
